...
class botinfo():
    ...
    async def botinfo(self,ctx, *, word=None):
        if not str(ctx.channel.id) in config.database.canais and not str(ctx.message.author.id) in config.database.admin:
           ...
           return
        if word is None:
           embed=discord.Embed(description=f"<:incorreto:518624535742906371> **|** Você não marcou um **BOT** para visualizar sua Informações.", color=0x7BCDE8)
        ...
        if bot is None:
           embed=discord.Embed(description=f"<:incorreto:518624535742906371> **|** o <@{numero}>** que você forneceu não está registrado em meu banco de dados!", color=0x7BCDE8)
           msg = await ctx.send(embed=embed)
           await asyncio.sleep(30)
           await msg.delete()
           return
