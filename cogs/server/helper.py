        ...
        try:
         try:
           ...
           if len(nome.content) >=40:              
              await msg.delete()
              embed=discord.Embed(description=f"<:incorreto:518624535742906371> **|** Olá **{ctx.author.name}**, o **Nome** que você inseriu passou do limite de 40 caracteres.", color=0x7BCDE8)
              msg = await ctx.author.send(embed=embed)
             ...
             if idade.content.isnumeric() == False:
                await msg.delete()
                embed=discord.Embed(description=f"<:incorreto:518624535742906371> **|** Olá **{ctx.author.name}**, a idade que você inseriu não é válida.", color=0x7BCDE8)
                msg = await ctx.author.send(embed=embed)
                await asyncio.sleep(30)
                await msg.delete()
             else:
               if len(idade.content) >=3:              
                  await msg.delete()
                  embed=discord.Embed(description=f"<:incorreto:518624535742906371> **|** Olá **{ctx.author.name}**, a idade que você inseriu não é válida. (10 anos a 99 anos.)", color=0x7BCDE8)
                  msg = await ctx.author.send(embed=embed)
                  await asyncio.sleep(30)
                  await msg.delete()
               else:
                 ...
                 if not str(lang1.content.lower()) in linguagem:                      
                   ...
                   if not str(lang2.content.lower()) in linguagem:                      
                    ...
                    if str(lang2.content.lower()) == str(lang1.content.lower()): 
                      await msg.delete()
                      embed = discord.Embed(description=f"<:incorreto:518624535742906371> **|** Olá **{ctx.author.name}**, a linguagem (**Secundária**) que você forneceu é igual a (**Primária**) e por isso a ação foi cancelada.", color=0x7BCDE8)
                      msg = await ctx.author.send(embed=embed)
                      await asyncio.sleep(30)
                      await msg.delete()
                    else:
                     ...
                     if len(motivo.content) <= 20:
                        await msg.delete()
                        embed=discord.Embed(description=f"<:incorreto:518624535742906371> **|** Olá **{ctx.author.name}**, o motivo é muito pequeno. (20 caracteres no mínimo)", color=0x7BCDE8)
                        msg = await ctx.author.send(embed=embed)
                        await asyncio.sleep(30)
                        await msg.delete()
                     else:
                       await msg.delete()
                       embed=discord.Embed(description=f"<:new:518620334052737034> **|** Olá **{ctx.author.name}**, abaixo está localizado as informações do seu cadastro caso tenha alguma coisa errada clique na reação (<:incorreto:518624535742906371>) para recusar e deletar, caso esteja certo clique na reação (<:correto:518624536082776084>).", color=0x7BCDE8)
                           ...
                           if reaction.emoji.name == 'correto':
                              ...
                              if users is None:
                                 ...
                                 if str(lang2.content) == "python":
                                   ...
                                   await msg.delete()
                                   embed = discord.Embed(description=f"<:incorreto:518624535742906371> **|** Diga-me o motivo da recusa do **Helper** ``{str(usuario)}``", color=0x7BCDE8)
                                   server = self.bard.get_guild(498011182620475412)
                                   channel = discord.utils.get(server.channels, id=527142906406895636)
                                   await channel.send(embed=embed)                                   
