        ...
        try:
         try:
           ...
           if id_bot.content.isnumeric() == False:
              await msg.delete()
              embed=discord.Embed(description=f"<:incorreto:571040727643979782>  **|** Olá **{ctx.author.name}**, o argumento que você inseriu não é um **ID** e por isso o cadastramento foi cancelado.", color=0x7BCDE8)
              msg = await ctx.author.send(embed=embed)
             ...
             try:
               ...
               if usuario.bot == True:
                ...
                usuario = None
             if usuario == None:
                embed=discord.Embed(description=f"<:incorreto:571040727643979782>  **|** Olá **{ctx.author.name}**, o **ID** que você inseriu não e invalído e por isso o cadastramento foi cancelado", color=0x7BCDE8)
                msg = await ctx.author.send(embed=embed)
                await asyncio.sleep(30)
                await msg.delete()
             else:
               ...
               if gg == 1:
                  ...
                  reactions = [":incorreto:571040727643979782", ':correto:518624536082776084']
                  ...
                  if reaction.emoji.name == 'incorreto':
                     await msg.delete()
                     embed=discord.Embed(description=f"<:incorreto:571040727643979782>  **|** o **ID** que você forneceu corresponde a de um **BOT** chamado `{usuario}`, já que **BOT** não é seu o cadastramento foi cancelado.", color=0x7BCDE8)
                     msg = await ctx.author.send(embed=embed)
                     await asyncio.sleep(30)
                     await msg.delete()
                  if reaction.emoji.name == 'correto':
                   ...
                   if len(prefix.content) +1 >= 8:                      
                      await msg.delete()
                      embed = discord.Embed(description=f"<:incorreto:571040727643979782>  **|** Olá **{ctx.author.name}**, o prefixo que você forneceu execedeu o limite máximo **(8)** caracteres e por isso ação foi cancelada.", color=0x7BCDE8)
                      msg = await ctx.author.send(embed=embed)
                      await asyncio.sleep(30)
                      await msg.delete()
                   elif str(prefix.content) in prefixos:                      
                      await msg.delete()
                      embed = discord.Embed(description=f"<:incorreto:571040727643979782>  **|** Olá **{ctx.author.name}**, o prefixo que você forneceu está banido e por isso ação foi cancelada.", color=0x7BCDE8)
                      msg = await ctx.author.send(embed=embed)
                      await asyncio.sleep(30)
                      await msg.delete()
                   else:
                     ...
                     if not str(lang.content) in linguagem:                      
                        await msg.delete()
                        embed = discord.Embed(description=f"<:incorreto:571040727643979782>  **|** Olá **{ctx.author.name}**, a linguagem que você forneceu é invalida e por isso ação foi cancelada.", color=0x7BCDE8)
                        msg = await ctx.author.send(embed=embed)
                        await asyncio.sleep(30)
                        await msg.delete()
                     elif str(lang.content) in linguagem:                      
                        await msg.delete()
                        embed = discord.Embed(description=f"<:new:518620334052737034> **|** Olá **{ctx.author.name}**, abaixo está localizado as informações do **BOT** caso tenha alguma coisa errada clique na reação (<:incorreto:571040727643979782> ) para recusar e deletar, caso esteja certo clique na reação (<:correto:518624536082776084>).",color=0x7BCDE8)
                        ...
                        if reaction.emoji.name == 'incorreto':
                           await msg.delete()
                           embed=discord.Embed(description=f"<:incorreto:571040727643979782>  **|** A solicitação foi cancelada.. tente novamente caso queira adicionar o bot novamente ao servidor.", color=0x7BCDE8)
                           msg = await ctx.author.send(embed=embed)
                           await asyncio.sleep(30)
                           await msg.delete()
                        if reaction.emoji.name == 'correto':
                           ...
                           if user == ctx.message.author:
                            ...
                            if reaction.emoji.name == 'incorreto':
                                  try:
                                   ...
                                   server = self.bard.get_guild(498011182620475412)
                                   channel = discord.utils.get(server.channels, id=507570211499671576)
                                   embed = discord.Embed(description=f"<:incorreto:571040727643979782>  **|** Diga-me o motivo da recusa do **BOT** ``{str(usuario)}``", color=0x7BCDE8)
