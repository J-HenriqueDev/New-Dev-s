           ...
           if id_bot.content.isnumeric() == False:
             ...
             try:
               ...
               if usuario.bot == True:
                ...
                usuario = None
             if usuario == None:
                embed=discord.Embed(description=f"<:incorreto:571040727643979782>  **|** Olá **{ctx.author.name}**, o **ID** que você inseriu não e invalído e por isso o cadastramento foi cancelado", color=0x7289DA)
                msg = await ctx.author.send(embed=embed)
                await asyncio.sleep(30)
                await msg.delete()
             else:
               ...
               if gg == 1:
                  ...
                  if reaction.emoji.name == 'correto':
                   await msg.delete()
                   txs = f"<:texto:576143949161562162> **|** Diga-nos agora o prefixo do seu **BOT** (máximo 8 caracteres)\n<:block:576119613385998338>Prefixo banidos **|** **[c.],[!],[/][@],[#]**\n{self.bard._emojis['timer']} **|** **2 minutos**"
                   embed=discord.Embed(description=txs, color=0x7289DA)
                   msg = await ctx.author.send(embed=embed)
                   ...
                   if len(prefix.content) +1 >= 8:                      
                     ...
                     if not str(lang.content) in linguagem:                      
                        ...
                        embed.add_field(name="<:bots:576143946867277845> Bot", value = "``"+str(usuario)+"``", inline=True)
                        embed.add_field(name="<:ip:576143947533910040> Id", value = "``"+str(usuario.id)+"``", inline=True)
                        embed.add_field(name="<:texto:576143949161562162> Prefixo", value = "``"+str(prefix.content)+"``", inline=True)
                        embed.add_field(name="<:api:576143946917478406> Linguagem", value = "``"+str(lang.content)+"``", inline=True)
                        ...
                        if reaction.emoji.name == 'correto':
                           ...
                           embed.add_field(name="<:texto:576143949161562162> Prefixo", value = "``"+str(prefix.content)+"``", inline=True)
                           embed.add_field(name="<:api:576143946917478406> Linguagem", value = "``"+str(lang.content)+"``", inline=True)
                           embed.add_field(name="<:mention:576143947013947403> Dono", value = "``"+str(ctx.author)+"``"+" ("+str(ctx.author.mention)+")", inline=True)
                           ...
                           if user == ctx.message.author:
                            ...
                            if reaction.emoji.name == 'correto':
                                   ...
                                   embed.add_field(name="<:api:576143946917478406> Linguagem", value = "``"+str(lang.content)+"``", inline=True)
                                   embed.add_field(name="<:mention:576143947013947403> Dono", value = "``"+str(ctx.author)+"``"+" ("+str(ctx.author.mention)+")", inline=True)
                                   embed.add_field(name="<:mention:576143947013947403> Aceito por", value = f"<@{author.id}>", inline=True)
                                   embed.set_thumbnail(url=usuario.avatar_url_as())
                                   embed.set_footer(text=self.bard.user.name+" © 2018", icon_url=self.bard.user.avatar_url_as())
                                   server = self.bard.get_guild(570906068277002271)
                            ...
                            if reaction.emoji.name == 'incorreto':
                                  try:
                                   ...
                                   if recused.content.lower().startswith("motivo :"):
                                       ...
                                       embed.add_field(name="<:api:576143946917478406> Linguagem", value = "``"+str(lang.content)+"``", inline=True)
                                       embed.add_field(name="<:mention:576143947013947403> Dono", value = "``"+str(ctx.author)+"``"+" ("+str(ctx.author.mention)+")", inline=True)
                                       embed.add_field(name="<:mention:576143947013947403> Recusado por", value = f"<@{author.id}>", inline=True)
                                       texto = str(recused.content).replace("motivo :", "")
                                       embed.add_field(name="<:block:576119613385998338>  Motivo", value = f"``{texto}``", inline=True)
                                       embed.set_thumbnail(url=usuario.avatar_url_as())
                                       embed.set_footer(text=self.bard.user.name+" © 2018", icon_url=self.bard.user.avatar_url_as())
                                       server = self.bard.get_guild(570906068277002271)
                                       channel = discord.utils.get(server.channels, id=571046221121060894)
